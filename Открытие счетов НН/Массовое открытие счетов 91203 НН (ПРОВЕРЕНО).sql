...
declare
vKey number;                --ключ
pFilialId varchar2(15) := '2188666'; --------------------------------------------------------------------------------------филиал НОМЕР
...
id_owner varchar2(10) :='2188666' ; --ИД владельца счета
...
select  
    nns as newnns
    ,'НН. Внебалансовый счет учета ключей по депозитарию ячейки № '||num_ibs as name_acc
    ,v91203 as bs_sys   
from sdm$ibs_nns
where 1=1
...
GC.INS_ACC(V_ACCNUM,v91203,'810',id_owner,null,type_acc,null,date_open,r.name_acc,V_AUTOCLEAR=>'N',NO_NNS=>false,pfilial=>pFilialId); --открытие счетов 91203
...
GC.SET_REKV(V_ACCNUM
           ,'810'
           ,'042202745'
           ,'30101810800000000745'
           ,'00000810100000000001'
           ,'ФИЛИАЛ "СДМ-БАНК" (ПАО) В Г.НИЖНИЙ НОВГОРОД'
           ,'ФИЛИАЛ "СДМ-БАНК" (ПАО) В Г.НИЖНИЙ НОВГОРОД'
           ,'НИЖНИЙ НОВГОРОД'
           ,V_UNB=>NULL
           ,V_SELF_MFO=>NULL
