...
declare
vKey number;                --ключ
pFilialId varchar2(15) := '185746585'; --------------------------------------------------------------------------------------филиал НОМЕР
...
id_owner varchar2(10) :='185746585' ; --ИД владельца счета
...
select  
    nns as newnns
    ,'ЕК. Внебалансовый счет учета ключей по депозитарию ячейки № '||num_ibs as name_acc
    ,v91203 as bs_sys   
from sdm$ibs_nns
where 1=1
...
GC.SET_REKV(V_ACCNUM
           ...
           ,'ФИЛИАЛ "СДМ-БАНК" (ПАО) В Г. ЕКАТЕРИНБУРГЕ'
           ,'ФИЛИАЛ "СДМ-БАНК" (ПАО) В Г. ЕКАТЕРИНБУРГЕ'
           ,'ЕКАТЕРИНБУРГ'
           ,V_UNB=>NULL
           ,V_SELF_MFO=>NULL
