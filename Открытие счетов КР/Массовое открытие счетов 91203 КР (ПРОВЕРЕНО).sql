...
declare
vKey number;                --ключ
pFilialId varchar2(15) := '1788509'; --------------------------------------------------------------------------------------филиал НОМЕР
...
id_owner varchar2(10) :='1788509' ; --ИД владельца счета
...
select  
    nns as newnns
    ,'КР. Внебалансовый счет учета ключей по депозитарию ячейки № '||num_ibs as name_acc
    ,v91203 as bs_sys   
from sdm$ibs_nns
where 1=1
...
GC.SET_REKV(V_ACCNUM
           ,'810'
           ,'040407862'
           ,'30101810500000000862'
           ,'00000810100000000001'
           ,'ФИЛИАЛ "СДМ-БАНК" (ПАО) В Г.КРАСНОЯРСКЕ'
           ,'ФИЛИАЛ "СДМ-БАНК" (ПАО) В Г.КРАСНОЯРСКЕ'
           ,'КРАСНОЯРСК'
           ,V_UNB=>NULL
           ,V_SELF_MFO=>NULL
